...
all_subs = which(meta_data$Subtype == "Delta")
...
bam_data_3 = read.table("~/Deko/Data/Human_Mouse_HSC/Yan.tsv" , sep ="\t" ,header = T, stringsAsFactors = F, row.names = 1)
...
bam_data_3 = bam_data_3[, meta_data$Subtype %in% c("HESC")]
dim(bam_data_3)
...
merge_genes = intersect(rownames(bam_data_2), rownames(bam_data_3))
length(merge_genes)
...
new_mat = new_mat[which( rowMeans(new_mat) >= 3),]
...
dim(new_mat)
new_mat[1:5,1:5]
#write.table(new_mat[,], "~/Deko/Data/Alpha_Beta_Gamma_Delta_Acinar_Ductyl_Segerstolpe.tsv", sep ="\t", quote =F , row.names = T)
