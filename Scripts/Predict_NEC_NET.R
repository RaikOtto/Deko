...
mtry = round(sqrt(ncol(cell_type_predictions)),0)
...
pred_nec_net = function(
    train_mat,
    truth_vec,
    model = ""
){
    if ( model == ""){
        NEC_NET_model = caret::train(
            x = train_mat,
            y = truth_vec,
            method = "rf",
            norm.votes=T,
...
randomizer_hold_out = 1:nrow(train_mat)
...
truth_vec = meta_info[rownames(train_mat),"NEC_NET"]

truth_vec_test = meta_info[rownames(train_mat[randomizer_test,]),"NEC_NET"]
truth_vec_hold_out = meta_info[rownames(train_mat[randomizer_hold_out,]),"NEC_NET"]

nec_net_model = pred_nec_net(
    train_mat = train_mat,
    truth_vec = truth_vec
    #train_mat = train_mat_test,
    #truth_vec = truth_vec_test
)


predict_statistics = predict(nec_net_model, train_mat)
truth_vec_statistics = factor(truth_vec, levels = c("NEC","NET"))
#predict_statistics = predict(nec_net_model, train_mat_hold_out)
#truth_vec_statistics = factor(truth_vec_hold_out, levels = c("NEC","NET"))

con_mat = confusionMatrix(
    predict_statistics,
    truth_vec_statistics,
    positive = "NEC"
)
