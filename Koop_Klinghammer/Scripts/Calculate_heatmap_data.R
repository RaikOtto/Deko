...
gene_table = read.table("~/Koop_Klinghammer/Misc/Stem_signatures.tsv",sep ="\t", header = F, stringsAsFactors = F)
dim(gene_table)
selected_genes = gene_table[30,3:ncol(gene_table)]
selected_genes = selected_genes[selected_genes != ""]
expr = pure_data[ rownames(pure_data) %in% selected_genes,]
selected_genes[ !( selected_genes %in% rownames(pure_data) ) ]
...
ggbiplot::ggbiplot(
  ...
  alpha = 1,
  groups = as.character( meta_data$Subtype_Kmeans ),
  ellipse = TRUE, 
  circle = TRUE,
  var.axes = F
...
os_vec = meta_data$OS[!(is.na(OS))]

OS = OS[!(is.na(OS))]
OS = OS[ (response_vec != "NE") & (response_vec != "")]
os_vec = os_vec[ (response_vec != "NE") & (response_vec != "")]
os_vec = as.double(os_vec)
Subtype = Subtype[ (response_vec != "NEmaxstat") & (response_vec != "")]

response_vec = response_vec[ (response_vec != "NE") & (response_vec != "")]
aggregate(os_vec, by = list(Subtype), FUN = mean)
library(survival)
fit <- survfit(
  Surv(OS) ~ Subtype
)
...
tab_mat = data.frame(cbind( response_vec, Subtype))
table(tab_mat)
...
t_test_data = data.frame( 
  "OS" = as.double(os_vec),
  "Subtype" = Subtype)
os_cl = t_test_data[ as.character(t_test_data[,2]) == "CL", 1]
os_ba = t_test_data[ as.character(t_test_data[,2]) == "BA", 1]
os_ms = t_test_data[ as.character(t_test_data[,2]) == "MS", 1]
