...
design <- model.matrix(~0 + as.factor(meta_data$Best_response))
colnames(design) = c("PD","PR","SD")

vfit <- lmFit(t(r_mat[,c(-1,-2)]),design)
#vfit <- contrasts.fit(vfit, contrasts=contr.matrix)
#plotSA(efit)

contr.matrix = makeContrasts( contrast = PR - SD,  levels = design )
vfit <- contrasts.fit( vfit, contrasts = contr.matrix)
efit <- eBayes(vfit)

summary(decideTests(efit))

result_t = topTable( efit, coef = "contrast", number  = ncol(r_mat), adjust  ="none", p.value = 1, lfc = 0)
...
write.table("~/Koop_Klinghammer/Results/Linear_regression/PR_vs_SD.tsv", x = result_t, sep = "\t", quote = F, row.names = F)
